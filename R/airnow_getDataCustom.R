...
#'
#' @title Download and parse custom data subset from the AirNow data API
#'
...
#' subsets of data. This function is useful when users want data for a small
#' subset of monitors within a particular region. It can also be used to access
#' international data for countries outside the Canada-US-Mexico default region.
#'
...
airnow_getDataCustom <- function(
  ...
  if ( MazamaCoreUtils::logger.isInitialized() )
    logger.debug(" ----- airnow_getDataCustom() ----- ")
  ...
  MazamaCoreUtils::validateLonsLats(c(bbox[1], bbox[3]), c(bbox[2], bbox[4]))
