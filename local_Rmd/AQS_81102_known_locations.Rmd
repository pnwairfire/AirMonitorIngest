---
title: "AQS 81102 Tutorial 2: Create 'known locations' Table"
...
This tutorial is a duplicate of AQS 81102 Tutorial 2 but using parameterCode
81102 instead. The result will be a "known locations" table for parameterCode 81102.

# Setup

In the first tutorial we explored AQS data and determined that there was 
agreement between the monitor and sites data associated with parameter 81102.
...
AQS_monitors_81102 <-
  AQS_monitors %>%
  # Subset
  dplyr::filter(`Parameter Code` == "81102") %>%
...
AQS_sites_81102 <-
  ...
  # Subset
  dplyr::filter(AQSID %in% AQS_monitors_81102$AQSID)
...
We will modify the `AQS_sites_81102` table using the 
...
Standardization of `AQS_sites_81102` will take place in several 
stages.
...
sites_locationTbl %>%
  dplyr::filter(locationID %in% duplicate_locationIDs) %>%
  dplyr::select(AQS_Address, AQS_Owning.Agency, AQS_Site.Number)
```

Some of the duplicates appear to be the same address with slightly different names.
Using Google maps to validate others, we some some addresses that do not match the
given longitude and latitude and are off by up to a mile in Seattle and Tacoma.

Correcting this is a manual task and beyond the scope of the current effort.
We will remove duplicates, trusting that the given longitude and latitude information
will adequately represent each "known location".
...
table_save(sites_locationTbl, "AQS_81102_sites")

table_csv <- table_export(sites_locationTbl)
readr::write_csv(sites_locationTbl, file = file.path(getLocationDataDir(), "AQS_81102_sites.csv"))
```

# Compare with 88101

Parameter code 81102 (PM2.5 FRM/FEM) measures PM2.5 while parameter code 81102
measure Ozone. Let's take at the known locations table for each.:

```{r compare_88101_81102}
...
kl_81102 <- MazamaLocationUtils::table_load("AQS_81102_sites")
...
The spatial distribution of PM10 and PM2.5 monitors shows a lot of spatial overlap.
