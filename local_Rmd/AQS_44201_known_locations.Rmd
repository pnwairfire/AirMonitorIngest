---
title: "AQS 44201 Tutorial 2: Create 'known locations' Table"
...
This tutorial is a duplicate of AQS 44201 Tutorial 2 but using parameterCode
44201 instead. The result will be a "known locations" table for parameterCode 44201.

# Setup

In the first tutorial we explored AQS data and determined that there was 
agreement between the monitor and sites data associated with parameter 44201.
...
AQS_sites_44201 <-
  ...
  # Subset
  dplyr::filter(AQSID %in% AQS_monitors_44201$AQSID)
...
We will modify the `AQS_sites_44201` table using the 
...
Standardization of `AQS_sites_44201` will take place in several 
stages.
...
The duplicates all appear to be locations within the same block. We choose to remove the 
duplicated sites on the assumption they represent repositioning at a "location"
that is identical from an atmospheric processes perspective.
...
table_save(sites_locationTbl, "AQS_44201_sites")

table_csv <- table_export(sites_locationTbl)
readr::write_csv(sites_locationTbl, file = file.path(getLocationDataDir(), "AQS_44201_sites.csv"))
```

# Compare with 88101

Parameter code 44201 (PM2.5 FRM/FEM) measures PM2.5 while parameter code 44201
measure Ozone. Let's take at the known locations table for each.:

```{r compare_88101_44201}
...
kl_44201 <- MazamaLocationUtils::table_load("AQS_44201_sites")
...
The spatial distribution of Ozone and PM2.5 monitors is different but no "story"
leaps out of this pattern.
