---
title: "AQS 88101 Tutorial 2: Create 'known locations' Table"
...
agreement between the monitor and sites data associated with parameter 88101.
...
We will modify the `AQS_sites_88101` table using the 
...
Standardization of `AQS_sites_88101` will take place in several 
stages.
...
# sites_locationTbl %>%
#   dplyr::filter(locationID %in% duplicate_sites) %>%
#   View()
...
The Washington University site has an `AQS_Site.Number` of `9010` but other
purely spatial data are identical to the Illinois EPA site. We can safely remove
the Wash. U. site.
...
table_save(sites_locationTbl, "AQS_88101_sites")

table_csv <- table_export(sites_locationTbl)
readr::write_csv(sites_locationTbl, file = file.path(getLocationDataDir(), "AQS_88101_sites.csv"))
```
