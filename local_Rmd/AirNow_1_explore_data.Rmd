...
airnow_data <- airnow_getHourlyData(datestamp)
```
...
need to appear in `aqs_sites`. Let's have a look at these records:
...
Not sure what the significance of these 3-digit prefixes is but all of these
sites appear to be in North America.

These records also have a lot of similarly bogus data. 
Corrupted fields include: `countryCode`, `FIPSStateCode`, `stateCode`, 
`GNISCountyCode`, `countyName`.
...
airnow_sites %>%
  ...
  MazamaLocationUtils::table_leaflet(locationOnly = TRUE)
```
