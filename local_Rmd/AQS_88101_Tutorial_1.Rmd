---
title: "AQS 88101 1: Explore AQS Data"
...
For this use case, we will ignore measurement scales < 100m.
...
We can create an interactive map showing the locations of the monitors (red) and 
sites (black ring) we have identified.
...
All red circles associated with `AQS_monitors` appear to be surrounded by black
rings associated with `AQS_sites`. This is as it should be as monitors with
different `POC` values may appear at the same site:
...
This ends the exploration of available data. See step 2 for the creation of a
"known locations" table for these locations.
