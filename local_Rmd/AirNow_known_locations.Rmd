---
title: "AirNow: Create 'known locations' Table"
...
**AirMonitorIngest** R packages to work with AirNow sites data. Target
...
42101 instead. The result will be a "known locations" table for parameterCode 42101.

# Setup

In the first tutorial we explored AQS data and determined that there was 
agreement between the monitor and sites data associated with parameter 42101.
...
AQS_monitors_42101 <-
  AQS_monitors %>%
  # Subset
  dplyr::filter(`Parameter Code` == "42101") %>%
...
AQS_sites_42101 <-
  ...
  # Subset
  dplyr::filter(AQSID %in% AQS_monitors_42101$AQSID)
...
We will modify the `AQS_sites_42101` table using the 
...
Standardization of `AQS_sites_42101` will take place in several 
stages.
...
The duplicates appear to be renamed or nearby addresses. We choose to remove the 
...
table_save(sites_locationTbl, "AQS_42101_sites")

table_csv <- table_export(sites_locationTbl)
readr::write_csv(sites_locationTbl, file = file.path(getLocationDataDir(), "AQS_42101_sites.csv"))
```

# Compare with 88101

Both parameterCode 88101 (PM2.5 FRM/FEM) and parameterCode 42101 (PM2.5 non-FRM/FEM)
...
kl_42101 <- MazamaLocationUtils::table_load("AQS_42101_sites")
